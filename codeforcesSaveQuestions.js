...
`;
  ...
  function createModal(problemId) {
    ...
    modal.querySelector("#createListBtn").addEventListener("click", () => {
  ...
  function addSaveButton() {
    const problemId = window.location.pathname.split("/").slice(-2).join("/");
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          try {
            ...
            GM_setValue(storageKey, importedData);
            location.reload();
          } catch (err) {
